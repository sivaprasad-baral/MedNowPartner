...
public class Login extends AppCompatActivity {
        ...
        firebaseAuth.signInWithCredential(FacebookAuthProvider.getCredential(accessToken.getToken())).addOnCompleteListener(Login.this, new OnCompleteListener<AuthResult>() {
            ...
            public void onComplete(@NonNull Task<AuthResult> task) {
                if (task.isSuccessful()) {
                    ...
                    databaseReference.child("Partners").child(firebaseUser.getUid()).addValueEventListener(new ValueEventListener() {
                        @Override
                        public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                            Partner partner = dataSnapshot.getValue(Partner.class);
                            databaseReference.child("Partners").child(firebaseUser.getUid()).removeEventListener(this);
                            if(partner == null) {
    ...
    private void addNewUserToDatabase() {
        Partner partner = new Partner(firebaseUser.getEmail(),firebaseUser.getDisplayName(),firebaseUser.getUid());
        partner.setProfileImg(Objects.requireNonNull(firebaseUser.getPhotoUrl()).toString());
        databaseReference.child("Partners").child(firebaseUser.getUid()).setValue(partner).addOnCompleteListener(new OnCompleteListener<Void>() {
            @Override
            public void onComplete(@NonNull Task<Void> task) {
                if(task.isSuccessful()) {
                    startActivity(new Intent(Login.this,PharmacyInfo.class).addFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_CLEAR_TASK));
                } else {
                    Toast.makeText(Login.this, Objects.requireNonNull(task.getException()).getMessage(),Toast.LENGTH_SHORT).show();
                    firebaseUser.delete();
                }
